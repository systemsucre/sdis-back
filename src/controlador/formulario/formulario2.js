...
import { Formulario2 } from "../../modelo/formulario/formulario2.js"
...
rutas.post("/listargestion", async (req, res) => {
    try {
        const resultado = await formulario2.listarGestion()
...
rutas.post("/listarmes", listarmes, async (req, res) => {
    ...
    try {
        const resultado = await formulario2.listarMes(req.body.id, req.body.fecha)
        return res.json({ data: resultado, ok: true })
    } catch (error) {
        console.log(error)
        return res.json({ msg: 'Error en el servidor', ok: false })
...
rutas.post("/listarvariable", id, async (req, res) => {
    try {
        const resultado = await formulario2.listarVariable(req.body.id, req.body.rol, req.body.ss) 
        return res.json({ data: resultado, ok: true })
    } catch (error) {
        console.log(error)
        return res.json({ msg: 'Error en el servidor', ok: false })
...
rutas.post("/listarindicadores", async (req, res) => {
    try {
        ...
        const resultado = await formulario2.listarIndicadores(datos)
    ...
    try {
        if (datos.length > 0) {
            ...
            datos.forEach(async e => {
                ...
                if (c === datos.length) {
                    const result = await formulario2.listarIndicadores(data)
