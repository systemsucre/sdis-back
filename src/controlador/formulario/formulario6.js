...
import { Formulario6 } from "../../modelo/formulario/formulario6.js"
import { id, listarmes, } from '../../validacion/formulario/formulario5.js'

import pool from '../../modelo/bdConfig.js'  

const rutas = Router()
const formulario6 = new Formulario6()



rutas.post("/listargestion", async (req, res) => {
    try {
        const resultado = await formulario6.listarGestion()
...
rutas.post("/listarmes", listarmes, async (req, res) => {
    ...
    try {
        const resultado = await formulario6.listarMes(req.body.id, req.body.fecha)
        return res.json({ data: resultado, ok: true })
    } catch (error) {
        console.log(error)
        return res.json({ msg: 'Error en el servidor', ok: false })
...
rutas.post("/listarvariable", id, async (req, res) => {
    try {
        const resultado = await formulario6.listarVariable(req.body.id, req.body.rol, req.body.ss) 
        return res.json({ data: resultado, ok: true })
    } catch (error) {
        console.log(error)
        return res.json({ msg: 'Error en el servidor', ok: false })
...
rutas.post("/listarindicadores", async (req, res) => {
    try {
        ...
        const resultado = await formulario6.listarIndicadores(datos)
...
rutas.post("/listarCabeceras", async (req, res) => {
    try {
        ...
        const resultado = await formulario6.listarCabeceras(datos)
    ...
    try {
        if (datos.length > 0) {
            ...
            datos.forEach(async e => {
                ...
                if (c === datos.length) {
                    const result = await formulario6.listarIndicadores(data)
