...
export class Formulario4 {
    ...
    listarVariable = async (gestion, rol, ss) => {
        const sql =
            ...
            where v.gestion = ${pool.escape(gestion)} and v.rol = ${pool.escape(rol)} and ss.id = ${pool.escape(ss)}
    ...
    listarIndicadores = async (datos) => {
        ...
        const sqlValores =
            `SELECT input, indicador, valor 
                FROM   valor
                where variable = ${pool.escape(datos.variable)} and mun = ${pool.escape(datos.smun)} and mes = ${pool.escape(datos.mes)}`;
